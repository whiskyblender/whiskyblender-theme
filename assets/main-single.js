...
const addQuantityListeners = () => {
  ...
  add.addEventListener('click', function () {
    ...
    let variant = getVariantFromSelectedOptions();
    let subtotal = document.querySelector(`.subtotal-of-items`);
    const strikePrice = subtotalDiv.querySelector('.strike-price');
    let priceText;
    if (strikePrice) {
      // If strike-price exists, get the next sibling text node
      priceText = strikePrice.nextSibling;
    } else {
      // If no strike-price, get the text node directly within subtotalDiv
      priceText = Array.from(subtotalDiv.childNodes).find(node => node.nodeType === Node.TEXT_NODE && node.textContent.trim());
    }
